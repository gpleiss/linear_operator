...
def _collapse_batch_and_singleton_dimensions(tensor, target_shape):
    # Collapse extra batch dims
    extra_batch_dims = tensor.dim() - len(target_shape)
    if extra_batch_dims > 0:
        tensor = tensor.sum(dim=list(range(extra_batch_dims)))

    # Collapse singleton dims
    singleton_dims = [
        i
        for i, (tensor_size, target_size) in enumerate(zip(tensor.shape, target_shape))
        if target_size == 1 and tensor_size > 1
    ]
    if len(singleton_dims):
        tensor = tensor.sum(dim=singleton_dims)
