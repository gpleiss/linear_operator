...
def _rank_one_update_to_cholesky(chol_factor: Tensor, vec: Tensor) -> Tensor:
    ...
    for k in range(n):
        ...
        chol_factor[..., k, k] = r
        if k < n - 1:
            chol_factor[..., (k+1):, k] = (chol_factor[..., (k+1):, k] + s * vec[..., (k+1):]) / c
            vec[..., (k+1):]= c * vec[..., (k+1):] - s * chol_factor[..., (k+1):, k]
...
class LowRankPlusDiagLinearOperator(SumLinearOperator):
    def __init__(self, *linear_operators: Tuple[Union[DiagLinearOperator, RootLinearOperator]], **kwargs):
        ...
        if isinstance(linear_operators[0], DiagLinearOperator):
            ...
            raise NotImplementedError("Must provide a DiagLinearOperator and a RootLinearOperator to LowRankPlusDiagLinearOperator")
